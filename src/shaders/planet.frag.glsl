...
void main()
{
    ...
    float continentBase = fbm(vPosition * 0.55);
    float coastDetail = fbm(vPosition * 4.0);
    float continent =
        continentBase +
    ...
    float mountainNoise = fbm(vPosition * 8.0);
    float mountains =
        landMask *
    ...
    float polarSnow =
        smoothstep(
            0.68,
            0.92,
            latitude + (polarVariation - 0.5) * 0.15
        );
    ...
    float sun = max(dot(normalize(vNormal), lightDir), 0.0);
    float nightMask = 1.0 - smoothstep(0.0, 0.20, sun);

    float diffuse =
        max(dot(normalize(vNormal), lightDir), 0.0);
